...
for (i in seq_along(y)){
  z_m<-mean(y[[i]])
  z_sd<-sd(y[[i]])
  z_se<-sd/sqrt(50) #this is throwing an error, preventing knitting
  z_lower_ci<- m+qnorm(0.5/2)*se #calculate the lower limit
  z_upper_ci<- m-qnorm(0.5/2)*se #calculate the upper limit
...
```{r}
survivors50CI <- survivors50[ , c(5,6)]
survivors50CI
